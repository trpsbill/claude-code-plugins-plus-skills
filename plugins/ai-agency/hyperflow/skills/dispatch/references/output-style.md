...
Profile: deep                   —           —
Agents                         12 agents  238.2k tokens
Wall-clock                      3m 47s
...
Backwards-compat: the shorter form (no Wall-clock / Cumulative rows) is acceptable for tasks with a single batch or a single agent — there's nothing to parallelise. For tasks with 2+ batches OR 2+ parallel-eligible workers, the two rows MUST appear.

Shorter example (single-batch task):

```
── Usage ─────────────────────────────────────────
11 agents   234.1k tokens
──────────────────────────────────────────────────
```
